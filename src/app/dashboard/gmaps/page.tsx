...
import { Search, Download, Plus, Globe, Phone, Trash2, Filter, Loader2, Database, MapPin, CheckSquare, Square, Check, X, ChevronDown, ChevronRight, Menu, Target, ExternalLink, Map } from 'lucide-react';
...
function GmapsLeadsContent() {
  ...
  const fetchLeads = async () => {
    let endpoint = '/api/leads?app=gmaps&';
  ...
  const fetchStats = async () => {
    const res = await fetch('/api/leads/stats?app=gmaps');
    const data = await res.json();
    setStats(data);
  };
  ...
  const handleExport = () => {
    let exportUrl = '/api/export?app=gmaps&';
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-3">
                <div className="flex items-center gap-2 text-indigo-500">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.4em]">Google Maps Extraction</span>
                </div>
                <h1 className="text-6xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">
                    Google <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-emerald-500">Maps Leads</span>
                </h1>
                <p className="text-slate-500 dark:text-slate-400 font-bold text-lg max-w-2xl">
                    Extract business intelligence instantly from Google Maps links with automated tagging.
                </p>
            </div>
...
export default function GmapsLeadsPage() {
  return (
