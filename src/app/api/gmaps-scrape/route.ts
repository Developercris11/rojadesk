...
import { scrapeGmapsLeads } from '@/lib/scraper/gmaps-scraper';
import prisma from '@/lib/prisma';

export async function POST(req: NextRequest) {
    try {
        const { url, category, state, city } = await req.json();
        console.log('API Request Received (GMaps):', { url, category, state, city });

        if (!url) {
            return NextResponse.json({ error: 'URL is required' }, { status: 400 });
        }

        const leads = await scrapeGmapsLeads(url);
        console.log(`Google Maps Scraper returned ${leads.length} leads`);
        ...
        return NextResponse.json({
            error: 'Failed to process Google Maps leads',
